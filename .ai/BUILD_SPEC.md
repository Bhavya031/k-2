...
Search on this page is exact-first over what the store already holds: a pass
number, a vendor name, a reference, an amount. This page answers exact questions
without a model call, and it stays that way. Do not add a chat box to it and do
not add a vector index.
...
## Stage 10 — Asking the ledger

A person who is not the accountant has questions the report page has no panel
for. What did we take from one quarry last month. Which vendors are unpaid past
their terms. Give me the unbilled list as a file I can send someone. Building a
panel for each is the wrong shape; the questions are open-ended and the store
already holds every answer.

So there is a second surface, separate from the offline report, where a person
asks in their own words. The offline report keeps its guarantee untouched: it
opens from disk with the network disabled and it calls no model. This one is
allowed to reach a model, because that is its whole purpose.

The division of labour is the same as everywhere else in this system. The model
decides which question to put to the store and how to phrase the answer. The
store computes every number. No figure a person reads is generated by a model,
and an answer that rests on rows names the rows it rested on, so a person can go
and look. Answering with a number that no query produced is the one failure this
surface must make impossible.

It answers in two shapes: on the screen for a question, and as a file to keep
when the person asks for one. It runs on the same configured provider as the
rest of the system, so it works on a subscription with no API key.

Done when: a question in plain words returns an answer whose every figure traces
to rows in the store, and a request for a file produces one.
