...
type PaymentLine = Readonly<{ vendor: string; accrualId: string; paperReference: string; grossPaise: number; tdsPaise: number; retentionPaise: number; netPaise: number; status: string }>;
...
function reportData(database: Database): ReportData {
  ...
  const lineRows = rows<PaymentLine & { runId: string }>(database, `
    SELECT p.payment_run_id AS runId, v.name AS vendor, p.accrual_id AS accrualId, a.paper_reference AS paperReference, p.gross_paise AS grossPaise,
...
export function buildReport(database: Database): string {
  const shippedSearch = searchExactFirst.toString();
